...
exports.rotate = async (event, context) => {
  ...
  const request = Buffer.from(event.data, 'base64').toString();
  console.log(`Requested rotation: ${request}`);
  if (request !== secret_name) {
    console.error(`Ignoring. Expected rotation for: ${secret_name}`);
    // While multiple secrets may be used on the same topic, it is OK to return
    // without an error here as a separate subscription should be used by the
    // function rotating the other secret.
    return;
  }
